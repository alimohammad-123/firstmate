...
Every ordinary Treehouse-backed ship or scout holds one process-independent lease from before endpoint creation until guarded cleanup conditionally returns the exact recorded path, lease id, and home-scoped holder.
A replacement endpoint verifies and reuses that lease, so endpoint, shell, backend, and machine restarts do not make the task copy allocatable.
...
A new tmux task also records its immutable `tmux_window_id=` for rollback and teardown while retaining the named `window=` target for captain-facing routing and presentation.
...
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux`, `herdr`, `zellij`, `orca`, or `cmux`), plus `jq` and the `treehouse` worktree provider for every session-provider-only backend (`tmux`, `herdr`, `zellij`, `cmux`).
`jq` parses ordinary Treehouse task lease identity on those four backends and already serves the experimental adapters' JSON paths; Orca's tool requirements remain unchanged.
...
A herdr, zellij, or cmux home is therefore never told `tmux` is missing, and the Treehouse task-lease identity upgrade check runs only for the backends that actually use Treehouse.
That check requires JSON durable acquire, JSON status, and lease-id plus holder conditional return as one supported surface.
