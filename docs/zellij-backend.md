...
Ordinary Zellij ship and scout copies use the shared durable Treehouse task-lease contract and therefore remain reserved independently of the Zellij process.
...
A pane can still disappear between verification and the operation; downstream submit, leased-path verification, and stale detection report that narrow race rather than treating exit 0 as success.
...
`pane_cwd` follows the top-level shell `cd` used to enter an already-acquired lease but not nested foreground subshell cwd changes.
Spawn therefore retains the stronger begin/end-marker `pwd` probe and requires its result to equal the exact leased path before launching a worker.
This active probe is scoped to spawn-time lease-path verification and is not advertised as a general live-cwd API.
...
- Exact leased-path entry verification requires the spawn-time marker probe.
