...
# Stronger primitive case: Zellij's `pane_cwd` JSON field stays frozen at
# wherever the pane's shell launched a nested foreground subshell. The active
# pwd probe intentionally remains correct for that harder command shape even
# though current spawn enters its durable lease with a top-level shell cd.
...
pass "real zellij: current_path tracks a NESTED SUBSHELL's own cd that a bare pane_cwd read cannot see"
