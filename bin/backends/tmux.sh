...
# the exact durable lease path after Treehouse returns its JSON identity.
#
...
# empty on any tmux error. Mirrors fm-spawn.sh's exact leased-path verification:
...
# composer verification - used for fixed spawn-time commands such as exact
# leased-path entry and the GOTMPDIR export that already use this sequence
...
fm_backend_tmux_kill() {  # <target>
  ...
  case "$target" in
    @*) tmux kill-window -t "$target" 2>/dev/null || true; return 0 ;;
    *:*)
      session=${target%%:*}
      window=${target#*:}
      ;;
...
# named target is absent, so the exact recorded window name or immutable window
# id must appear in a successful inventory before its foreground command can be
# trusted. An omitted window or a definitive missing-session/server response is
...
fm_backend_tmux_agent_state() {  # <target>
  ...
  if ! printf '%s\n' "$windows" | grep -Fqx "${window:-$target}"; then
    printf 'missing'
    return 0
  fi
