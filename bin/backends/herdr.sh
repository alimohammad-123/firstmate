...
# any error. `pane get`'s `.result.pane.cwd` is creation-time-frozen and does not
# follow even the top-level shell `cd` used to enter a durable Treehouse lease.
# `.result.pane.foreground_cwd` tracks the running shell and is therefore the
# exact postcondition fm-spawn verifies before launching a worker.
...
# spawn-time commands such as exact leased-path entry and GOTMPDIR export. `pane run` types
