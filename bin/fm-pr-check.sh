...
# exact pr_head=<sha> when available under the task lifecycle mutation lock,
...
pr_check_cleanup() {
  ...
  [ -z "$META_MUTATION_LOCK" ] || fm_lock_release "$META_MUTATION_LOCK" || true
}
...
[ -f "$META" ] && [ ! -L "$META" ] && [ "$(fm_pr_file_link_count "$META")" = 1 ] || exit 1
