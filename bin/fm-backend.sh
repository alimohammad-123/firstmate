...
#   - jq plus the Treehouse worktree provider for every session-provider-only
#     backend (tmux, herdr, zellij, cmux), because ordinary task lease identity
#     is JSON; Orca owns its own task worktree and terminal, so its tool delta
#     remains unchanged.
